...
def gui_launch(
    ...
    """Launch the Transformer Lab GUI application."""
    ...
    if not run_script.exists():
        console.print(
            ...
            "Run [bold]lab gui update[/bold] to install it."
        )
        raise typer.Exit(1)
    ...
    console.print("[bold green]Launching Transformer Lab GUI...[/bold green]")
    try:
        subprocess.run(args, cwd=str(src_dir), check=True)
    except subprocess.CalledProcessError as e:
        console.print(f"[red]Error:[/red] Failed to launch GUI: {e}")
        raise typer.Exit(1)


@app.command("update")
def gui_update() -> None:
    """Update (or install) the Transformer Lab GUI by running the install script."""
    console.print("[bold green]Updating Transformer Lab GUI...[/bold green]")
    try:
        subprocess.run(
            ["bash", "-c", "curl -fsSL https://lab.cloud/install.sh | bash"],
            check=True,
        )
        console.print("[green]✓[/green] Transformer Lab GUI updated successfully.")
    except subprocess.CalledProcessError as e:
        console.print(f"[red]Error:[/red] GUI update failed: {e}")
        raise typer.Exit(1)
