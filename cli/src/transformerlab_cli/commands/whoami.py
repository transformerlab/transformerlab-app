...
# Import the configured console instead of creating a new generic one
...
def whoami():
    ...
    api_key = get_api_key()
    if not api_key:
        # USAGE: Use [error] style defined in theme
        console.print("[error]Error:[/error] Not logged in. Please run 'lab login' first.")
        raise typer.Exit(1)
    ...
    # Fetch user info
    # USAGE: Use [info] style
    with console.status("[info]Fetching user information...", spinner="dots"):
        user_info = fetch_user_info(api_key)
        teams_info = fetch_user_teams(api_key)

    if not user_info:
        console.print(
            "[error]Error:[/error] Could not fetch user information. Please check your API key and server connection."
        )
        raise typer.Exit(1)
    ...
    if cli_state.output_format == "json":
        ...
        # USAGE: Use "label" and "value" styles
        ...
        if teams_info and teams_info.get("teams"):
            ...
            if current_team_id:
                ...
                if current_team:
                    table.add_row(
                        "Current Team", f"{current_team.get('name', 'N/A')} ({current_team.get('role', 'N/A')})"
                    )
