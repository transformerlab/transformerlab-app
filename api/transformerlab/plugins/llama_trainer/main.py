...
from transformers import AutoConfig, AutoModelForCausalLM, AutoTokenizer, Mxfp4Config  # noqa: E402
...
def train_model():
    ...
    if not HAS_AMD:
        if "gpt-oss" in model_id:
            print(
                "Training GPT-OSS model is only supported for GPUs with compute capability 9.0 or higher."
            )
            quantization_config = Mxfp4Config(dequantize=True)
            print(
                "The model is dequantized during loading. Please ensure you have enough VRAM for loading the model."
            )
    ...
    try:
        ...
        print(f"Model loading error: {e!s}")
        raise
    ...
    try:
        ...
        print(f"PEFT model preparation error: {e!s}")
        peft_config = LoraConfig(
            lora_alpha=lora_alpha,
            lora_dropout=lora_dropout,
            r=lora_r,
    ...
    try:
        ...
        print(f"Model saving error: {e!s}")
        raise
    ...
    if tlab_trainer.params.get("fuse_model", False):
        ...
        try:
            ...
            tlab_trainer.create_transformerlab_model(
                ...
                model_architecture,
                json_data,
                parent_model=tlab_trainer.params.model_name,
            )

        except Exception as e:
            print(f"Model merging error: {e!s}")
