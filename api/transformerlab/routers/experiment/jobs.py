...
async def get_tunnel_info_for_job(
    ...
    # so it survives log rotation / truncation.
    if tunnel_info.get("is_ready"):
        await job_service.job_update_job_data_insert_key_value(job_id, "cached_tunnel_info", tunnel_info, experimentId)
        print(f"[tunnel_info] Job {job_id}: cached tunnel_info to job_data")
    elif not tunnel_info.get("is_ready"):
        # Logs no longer contain the URLs — fall back to previously cached info.
        cached = job_data.get("cached_tunnel_info")
        if isinstance(cached, dict) and cached.get("is_ready"):
            tunnel_info = cached
            print(f"[tunnel_info] Job {job_id}: using cached tunnel_info from job_data")
        else:
