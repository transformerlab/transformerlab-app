...
import { Box, Card, Typography, Chip, Divider, Accordion, AccordionSummary, AccordionDetails, TabList, Tabs, Tab, TabPanel } from '@mui/joy';
    ...
    <Box sx={{ width: '100%' }}>
      {provenance.provenance_chain.map((step, index) => (
        <Box key={step.job_id} sx={{ mb: 3 }}>
          {/* Timeline connector */}
          {index > 0 && (
            <Box sx={{ display: 'flex', alignItems: 'center', height: '24px', ml: 4, mb: 1 }}>
              <Box sx={{ width: '2px', height: '100%', bgcolor: 'neutral.300' }} />
            </Box>
          )}
          ...
          >
            <Box sx={{ p: 2 }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2, alignItems: 'center' }}>
                ...
                <Chip size="sm" variant="soft">{step.job_id}</Chip>
              </Box>

              {/* Key model info */}
              <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 2 }}>
                <Box>
                  <Typography level="body-sm" color="neutral">Base Model</Typography>
                  <Chip variant="soft" color="primary">{step.input_model}</Chip>
                </Box>

                <Box>
                  <Typography level="body-sm" color="neutral">Training Dataset</Typography>
                  <Chip variant="soft" color="success">{step.dataset}</Chip>
                </Box>

                {/* Key parameters summary */}
                <Box>
                  <Typography level="body-sm" color="neutral">Key Parameters</Typography>
            ...
            <Accordion
              ...
              onChange={() => setExpandedStep(expandedStep === step.job_id ? null : step.job_id)}
            >
              <AccordionSummary>
                <Typography level="body-sm">
                  {expandedStep === step.job_id ? "Hide details" : "Show details"}
                </Typography>
              </AccordionSummary>
              <AccordionDetails>
  <Tabs defaultValue="params">
    ...
    <TabPanel value="params">
      <Box sx={{ maxHeight: '300px', overflow: 'auto' }}>
        <Box component="pre" sx={{ fontSize: '0.75rem', p: 2, bgcolor: 'background.level1', borderRadius: '4px' }}>
      ...
      {step.evals && step.evals.length > 0 ? (
        <Box sx={{ maxHeight: '300px', overflow: 'auto' }}>
          {step.evals.map(evalItem => (
            <Card key={evalItem.job_id} variant="outlined" sx={{ mb: 2, p: 2 }}>
              <Typography level="title-sm">
                {evalItem.template_name || evalItem.evaluator || 'Evaluation'} ({evalItem.job_id})
              </Typography>
              <Divider sx={{ my: 1 }} />
              ...
              {evalItem.scores ? (
                <Box>
                  {(() => {
                    try {
                      const scoresArray = JSON.parse(evalItem.scores);
                      return Array.isArray(scoresArray) ? (
                        scoresArray.map((scoreObj, idx) => (
                          <Box key={idx} sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                            <Typography level="body-sm">{scoreObj.type || 'Metric'}:</Typography>
                        ...
                        <Typography level="body-sm">Invalid score format</Typography>
                      );
                    } catch (error) {
                      return (
                        <Typography level="body-sm">Error parsing scores</Typography>
                      );
                    }
                  })()}
                </Box>
              ) : (
                <Box component="pre" sx={{ fontSize: '0.75rem', mt: 1, p: 1, bgcolor: 'background.level1', borderRadius: '4px' }}>
...
function extractKeyParams(params) {
  ...
  const importantKeys = [
    ...
    'lora_rank', 'lora_alpha', 'lora_dropout', "type"
  ];
