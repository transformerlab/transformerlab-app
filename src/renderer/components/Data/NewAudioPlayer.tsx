...
const AudioPlayer: React.FC<AudioPlayerProps> = ({
  ...
  const [resampledAudio, setResampledAudio] = React.useState<string | undefined>(undefined);
  ...
  // Many audio files in datasets are not at standard sampling_rates.
  // If we try to play them they will sound slow or fast.
  // We need to resample the Base64 encoded string to 44100 Hz if necessary.
  const resampleAudio = async (base64Audio: string, originalRate: number, targetRate: number = 44100) => {
  ...
  // Compact Mode - resample the audio if needed
  // If no sampling_rate is included with the data then assume standard already.
  React.useEffect(() => {
    const resampleIfNeeded = async () => {
      console.log("FOR REAL");
      if (compact && metadata?.sampling_rate && metadata?.sampling_rate != 44100) {
        console.log("FIXING RATE");
        const resampledAudioUrl = await resampleAudio(audioData.audio_data_url, metadata.sampling_rate);
        setResampledAudio(resampledAudioUrl);
      } else {
        console.log("STANDARD RATE");
        setResampledAudio(audioData?.audio_data_url);
      }
    };
    resampleIfNeeded();
  }, [compact, audioData?.audio_data_url]);
