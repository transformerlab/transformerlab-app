...
Ask an open-source music model for "a lo-fi hip-hop beat at 90 BPM in the key of C major" and you get back something that sounds like lo-fi hip-hop. Whether it is actually at 90 BPM, or in C major, is a different question. Our lab measured it across three open instrumental generators, and the answer separates them more than sound quality does.

<!--truncate-->

> **Quick summary.** A recent vendor report ranks three open-source instrumental music generators by audio quality, using two metrics from one model family. We re-ranked the same three models, Stable Audio 3 Medium, ACE-Step 1.5, and DiffRhythm 2, on a wider stack of nine metrics, and the reported winner (Stable Audio 3) holds up: it places first on all nine, including metrics built on completely different models. Then we tested something the quality scores never check, whether a model obeys an explicit tempo and key. Here the three models split apart. Stable Audio 3 hits the requested tempo on 61% of clips and the exact key on 64%. ACE-Step sits at chance on key and barely above it on tempo. That controllability gap is the one that matters for real work, and it is invisible to the benchmarks these reports use. One honest limit up front: every number here is an automated score, not a human listening test, which is the study's main caveat.
...
The second question is the one the reports skip entirely. A music model is useful in a real pipeline when you can tell it what to make: this tempo, this key, so it slots under a vocalist or cuts to a video. None of FAD, CLAP, or the usual quality scores check whether the model did what it was told.
