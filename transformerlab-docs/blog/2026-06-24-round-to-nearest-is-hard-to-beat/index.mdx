...
import StreamingMemoryDiagram from './StreamingMemoryDiagram';
import RepetitionLoopDiagram from './RepetitionLoopDiagram';

_We tried to beat an open coding model's stock 4-bit build so it would run better on a 24 GB Mac. Nothing did, and the bottleneck turned out not to be the bits._

> **Quick summary:** We wanted the best coding model that fits in 24 GB on an Apple Silicon Mac. We started from North-Mini-Code, a 30-billion-parameter open model (Apache-2.0), and tried roughly a dozen ways to quantize it past its stock 4-bit version: cheaper formats, more expensive ones, custom per-part bit budgets, and a calibrated method we had to build new tooling just to run. None of them beat plain round-to-nearest 4-bit on coding tests. The 4-bit model already fits in 20.81 GB at a realistic working context and decodes at about 40 tokens per second, so memory and speed were never the real limit. The real limit only showed up when we ran the model as an agent: it often falls into repetition loops and never finishes the task. That is a decoding problem, not a quantization problem.

<!--truncate-->

## We wanted a coding agent that runs locally
...
A cheap attempt to distill the model into a smaller dense one (training a smaller student to imitate the big model) failed its early checkpoint outright, scoring 0 out of 32. So the best model you can run in 24 GB is the stock 4-bit build.
...
## The things that tied or lost (the useful part)

The whole study is a negative result. That is what makes it useful. Mixed precision, per-expert budgets, calibrated GPTQ, and distillation each tied or lost to a method with no calibration data and no tuning. Reporting that plainly is more useful than a cherry-picked win, because it tells you where not to spend effort on this class of model.

## Where this does not apply

This is one model, on one runtime (MLX), under one memory budget. We never measured the original higher-precision model (8-bit or 16-bit), so we cannot call 4-bit lossless, only say that nothing we tried beat it. We also held the router and attention at full precision and did not calibrate the router (the part that picks which experts handle each token), which the literature rates as the most important lever, so a different setup could move the result. And the agentic number is a lower bound from a weak harness.
