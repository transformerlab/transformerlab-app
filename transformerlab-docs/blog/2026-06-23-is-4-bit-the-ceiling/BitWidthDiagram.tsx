...
const CONFIGS: Config[] = [
  {
    key: 'm34',
    label: '3.5-bit mix',
    bits: '~3.5 bits/weight',
    score: 0.8598,
    mem: 14.81,
    verdict: 'Below 4-bit: a real accuracy drop, even though it saves memory.',
    good: false,
  },
  {
    key: 'rtn',
    label: '4-bit',
    bits: '4.0 bits/weight',
    score: 0.8902,
    mem: 19.33,
    verdict: 'The deliverable. Fits 24 GB with room to spare.',
    good: true,
  },
  {
    key: 'sgptq',
    label: '4-bit, calibrated',
    bits: '4.0 bits/weight',
    score: 0.8841,
    mem: 19.33,
    verdict:
      'Calibration (GPTQ) ties plain 4-bit exactly (p = 1.000). No gain.',
    good: true,
  },
  {
    key: 'm48',
    label: '5.2-bit mix',
    bits: '~5.2 bits/weight',
    score: 0.9024,
    mem: 20.53,
    verdict: 'Ties 4-bit (p = 0.79). The +1.2-point lead is inside the noise.',
    good: true,
  },
];
...
function pctScore(s: number): number {
  return Math.max(
    0,
    Math.min(100, ((s - SCORE_MIN) / (SCORE_MAX - SCORE_MIN)) * 100),
  );
}
...
export default function BitWidthDiagram(): React.ReactElement {
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.row}>
        ...
        <div className={styles.scaleRow}>
          ...
          <span className={styles.bandLabel}>
            shaded = tie with 4-bit (within noise)
          </span>
      ...
      <div className={styles.row}>
        <div className={styles.metricLabel}>
          Peak memory (of the 24 GB budget)
        </div>
        <div className={styles.track}>
          <div
      ...
      <p className={styles.caption}>
        Step across the settings. Below 4-bit the score falls out of the shaded
        tie band. Above 4-bit it stays inside the band while the memory bar
        climbs. 4-bit is the sweet spot: less costs accuracy, more costs only
        memory.
      </p>
    </div>
  );
