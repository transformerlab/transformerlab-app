...
(function (root, factory) {
  ...
  function loc(p, a, b) { return clamp((p - a) / ((b - a) || 1e-6), 0, 1); }

  var U = {
    clamp: clamp, smooth: smooth, lerp: lerp,
    sub: loc,
    ease: function (p, a, b) { return smooth(loc(p, a, b)); },
    at: function (p, a, b) { return p >= a && p < b; },
    count: function (p, a, b, n) { return Math.round(loc(p, a, b) * n); },
    // value keyframes: kf(p, [[t0,v0],[t1,v1],...]) — eased between stops
    kf: function (p, stops) {
      if (p <= stops[0][0]) return stops[0][1];
      for (var i = 1; i < stops.length; i++) {
        if (p <= stops[i][0]) return lerp(stops[i - 1][1], stops[i][1], smooth(loc(p, stops[i - 1][0], stops[i][0])));
      }
  ...
  function Stage(section, opts) {
    ...
    this.stage.style.cssText = 'position:sticky;top:' + this.top + 'px;height:calc(100vh - ' + this.top + 'px);overflow:hidden';
    ...
    var self = this;
    requestAnimationFrame(function () { self._draw(); });   // first paint after consumer builds DOM
  }
  Stage.prototype._tick = function () { if (this._q) return; this._q = true; var s = this; requestAnimationFrame(function () { s._q = false; s._draw(); }); };
  ...
  Stage.prototype._draw = function () { this.p = this.progress(); this.render(this.p, U); };
  Stage.prototype.refresh = function () { this._draw(); };
  ...
  return { create: function (s, o) { return new Stage(s, o); }, U: U };
}));
