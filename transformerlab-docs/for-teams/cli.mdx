...
:::warning
Do not run `server install` from inside `~/.transformerlab/src` — that directory is deleted during installation. Change to a different directory first (`cd ~ && lab server install`).
:::
